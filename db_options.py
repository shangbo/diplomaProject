...
def db_store_request(conn, info):
    cursor = conn.cursor()
    write_sql = '''insert into requests(root, method, _date, md5, url, keys_values, sql_injection) values(%s, %s, %s, %s, %s, %s, 0)''' 
    cursor.execute(write_sql, info)
    conn.commit()
    cursor.close()

def db_check_md5(conn, md5):
    cursor = conn.cursor()
    read_sql = "select * from requests where md5=%s"
    cursor.execute(read_sql, md5)
